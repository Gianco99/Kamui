...
- `expr` is C++ evaluated at job runtime, so `check` cannot catch a typo. `bDiscriminator()` returns -1000 for a missing tagger, `electronID()` throws. `cmssw/inspectMiniAOD.py` lists what a file carries.
