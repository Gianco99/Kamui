...
`./kamui check` enforces it by scanning for `paths.*_DIR` and `paths.SITES_FILE` outside `python/kamui/configReaders/`. This is why `loadSites` lives in `configReaders/sites.py`.
