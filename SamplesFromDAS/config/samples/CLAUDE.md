...
## Sample Caveats
Physics caveats per family are kept here rather than in the JSON so the configs stay readable.
...
- Private from Bruno. 
- Exclusive per (mStop, mSo, ctau), unlike the Run 2 inclusive RandPars samples.
...
`--family`, `--era` and `--tag` are resolved case-insensitively against what exists in the catalog. A value matching nothing raises rather an error and names every value that does exist. 
- `--tag` values are free strings. 
